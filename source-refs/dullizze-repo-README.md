...
| **Mark** | A 26 px rounded square with a 135° gradient from `--brand-teal` (#0F766E) → `--brand-blue` (#2563EB) |
| **Primary colour** | Teal `#0F766E` (used on every primary CTA and on the brand mark) |
| **Voice** | Friendly, plain Korean. Reassuring during long waits. Never developer-facing. |
| **Layout vibe** | shadcn-style: white cards on a warm grey background, 1 px borders, 6–8 px radii, low-contrast shadows. Calm and clerical. |
...
- **Primary palette is teal-on-warm-grey.** Brand teal (`#0F766E`) is the
  only colour that lives on CTAs and on the brand mark; everything else
  in the chrome is neutral. We never use teal for body text — it stays a
  CTA / selection / focus signal.
- **Surfaces.** Page background is `--bg` (`#F6F7F9`, a faintly cool warm
  grey). Cards sit on top in pure `#FFFFFF` with a 1 px `--line` border.
  We never use shadow alone to separate a card from the page — there is
  always a border too.
- **Status colours map 1:1 to job state.** `queued` / `running` → amber
  (`#B7791F`); `done` → teal; `failed` → red (`#B42318`). These are the
  ONLY semantic colours, and the badge backgrounds are washed-out tints
  (`--*-soft`), never saturated.
...
- No gradients in product chrome. The **single exception** is the brand
  mark (teal→blue, 135°). The video templates use their own protection
...
- **Hover** on a primary button → background darkens from `--accent`
  to `--accent-hover` (teal-700). No scale, no shadow change.
- **Hover** on a secondary / ghost button → background tints to
  `--surface-soft` (grey-100).
- **Focus** ring = 3 px `--accent-ring` (teal at 14% alpha) outside the
...
- **Logo**: a single `dullizze-mark.svg` (the teal→blue gradient square)
